...
ADMIN_IDS = [int(x.strip()) for x in os.environ.get("TELEGRAM_ADMIN_IDS", "7658500284,6533708508,5070917870,8123065501,7604152165").split(",")]
ADMIN_USERNAMES = {"redbullusernames", "ser0ton9num", "redbullusernamez"}
BLOCKED_ADMINS = {"kam_hr"}
DATABASE_URL = os.environ.get("NEON_DATABASE_URL") or os.environ.get("DATABASE_URL")


def is_admin(user) -> bool:
    if user.username and user.username.lower() in BLOCKED_ADMINS:
        return False
...
# --- Определение режима деплоя ---
# Поддерживаем Fly.io (FLY_APP_NAME), Replit (REPLIT_DEPLOYMENT) и ручной WEBHOOK_HOST
_fly_app = os.environ.get("FLY_APP_NAME", "").strip()
_replit_deployed = os.environ.get("REPLIT_DEPLOYMENT") == "1"
_replit_domain = os.environ.get("REPLIT_DOMAINS", "").split(",")[0].strip() if _replit_deployed else ""
_manual_host = os.environ.get("WEBHOOK_HOST", "").strip()

if _manual_host:
    WEBHOOK_HOST = _manual_host
elif _fly_app:
    WEBHOOK_HOST = f"{_fly_app}.fly.dev"
elif _replit_domain:
    WEBHOOK_HOST = _replit_domain
else:
    WEBHOOK_HOST = None
...
_bot_session = AiohttpSession(timeout=aiohttp.ClientTimeout(total=30, connect=10))
bot = Bot(token=TOKEN, parse_mode=ParseMode.HTML, session=_bot_session)
dp = Dispatcher()
...
SQUADS = {
    "q": ("qwn squad", "https://t.me/+gp-q9cbyRHxlY2I6"),
    "r": ("red's squad", "https://t.me/+pcUFwnYm2rtjYzky"),
    "s": ("saint squad", "https://t.me/+EaJrg1WAxXBlZDcy"),
}
...
def validate_operator_form(text: str) -> list:
...
async def check_ai_generated(text: str) -> tuple:
    try:
...
async def db_get_slot_text(slot_id: int):
    async with db_pool.acquire() as conn:
...
def extract_scout_username(app_text: str):
    import re
...
def extract_form_field(app_text: str, *keywords):
...
def admin_notify_keyboard(app_type: str, app_id: int):
    ...
    if app_type == "scout":
        rows.append([
            InlineKeyboardButton(text="🟢 Qwn", callback_data=f"sq_ap:q:{app_id}"),
            InlineKeyboardButton(text="🔴 Red's", callback_data=f"sq_ap:r:{app_id}"),
...
async def callbacks(callback: types.CallbackQuery):
    ...
    if callback.data.startswith("del_cancel:"):
        ...
        if not apps:
            try:
                await callback.answer()
                await callback.message.edit_text("📭 Заявок больше нет.", reply_markup=None)
            except Exception:
                pass
            return
...
async def _resolve_admin_usernames():
    for uname in ADMIN_USERNAMES:
        try:
            chat = await bot.get_chat(f"@{uname}")
            if chat.id not in ADMIN_IDS:
                ADMIN_IDS.append(chat.id)
                logger.info(f"Добавлен админ @{uname}: {chat.id}")
        except Exception as e:
            logger.warning(f"Не удалось найти @{uname}: {e}")
...
async def _webhook_background_init(db_url: str):
    try:
        ...
        return
    await _resolve_admin_usernames()
    try:
...
async def main():
    ...
    if WEBHOOK_URL:
        logger.info(f"Режим: Webhook (production) → {WEBHOOK_URL}")
        ...
        await runner.setup()
        site = web.TCPSite(runner, "0.0.0.0", 8080, backlog=256)
        await site.start()
