...
# OpenAI отключён — функция проверки на ИИ не используется
...
ADMIN_USERNAMES = { "ser0ton9num", "paffi_lov"}
...
WEBHOOK_HOST = os.environ.get(“REPLIT_DOMAINS”, “”).split(”,”)[0].strip() if IS_DEPLOYED else None
...
# AI-проверка отключена — всегда пропускаем анкету
return False, “”
...
f”{’’.join(traceback.format_exception(type(event.exception), event.exception, event.exception.**traceback**))}”
)
return True
...
if WEBHOOK_URL:
    ...
    await site.start()
    logger.info("Сервер запущен на порту 8080 — health check пройдёт")
